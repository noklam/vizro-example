...
This guide shows you how to create custom charts and how to add them to your dashboard. The Graph model accepts the `figure` argument, where you can enter *any* [`plotly.express`](https://plotly.com/python/plotly-express/) chart as explained in the [user guide on graphs](../graph/).
...
1. The visualization should be derived from and require only one `pandas.DataFrame`. Dataframes from other arguments will not react to dashboard controls such as [`Filter`](../filters/).
1. Pass your function to the `figure` argument of the Graph model.

The minimal example below can be used as a base to build more sophisticated charts.

Minimal example of a custom chart
...
To alter the data in the `data_frame` argument, consider using a [Filter](../filters/) or [parametrized data loading](../data/#parametrize-data-loading) and [dynamic data](../data/#dynamic-data). The `data_frame` argument input to a custom chart contains the data **after** filters and parameters have been applied.

Note

Custom charts can be targeted by [Filters](../filters/) or [Parameters](../parameters/) without any extra configuration. We will showcase both possibilities in the following examples.

## Interact with other graphs and tables

A custom graph can act as a source for [interactions with other components](../graph-table-actions/), for example to cross-filter another graph or table when the user clicks on a point.
...
Custom `plotly.express` scatter chart with a `Parameter`
...
Custom `go.Figure()` waterfall chart with a `Parameter`
